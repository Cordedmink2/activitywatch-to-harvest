...
3. Fire one capture by hand and confirm the files land: run `screenshot_capture.py` from the sibling skill's `scripts/`, then list the configured screenshot directory for today and confirm `HH-MM-SS_m1.png` files appeared, one per monitor.
...
Nothing in this skill posts to a timesheet provider, and the `daily` skill will not either without an explicit confirmation. Say that too — it is the question a new user has and does not always ask.
