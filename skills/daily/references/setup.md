...
The user filled in `/plugin configure billables`, started a new session, and a script still says the value is missing. Three causes, in the order worth checking. **The first two look identical to the user and need opposite things**, so establish which shell the failing command came from before recommending anything.

**1. The command did not come from the Bash tool.** The values are published as a POSIX shell fragment, and Claude Code applies that to **Bash** tool calls alone — its PowerShell tool is given no equivalent and loads no profile. So a script the model happens to run through PowerShell reports the setting missing on a machine that is configured perfectly well, and the *hook ran correctly*: this is not the cause below and Git Bash is already installed. Fix: re-run the same command through the Bash tool. The error says so itself when it can tell — it names the Bash tool whenever the failing process was a Windows one inside a session with no POSIX shell around it. Nothing needs restarting and nothing needs installing. Confirmed empirically: a value published by a SessionStart hook read back as set in a Bash tool call and unset in a PowerShell one, in the same session. The lasting fix is to read configured values in Bash and hand PowerShell the literal — `SKILL.md` "Running the scripts" carries it.
...
Cause 1 and cause 3 are not the same gap and share no fix. In 3 the hook never started, so nothing was published to any shell; in 1 the hook ran and wrote the fragment correctly, and one shell was not given it. `sh --version` separates them in one command: it answers in 1 and fails in 3.
