...
| No assumed timezone: an unconfigured run refuses rather than guessing an offset | `scripts/aw_client.py` `resolve_zone()` docstring | `tests/test_declared_configuration.py` — the unconfigured user gets a refusal naming both ways to supply a zone, never an offset | `SKILL.md` "Timezone", `references/activitywatch.md` §"Time zones", both scripts' module docstrings |
| A day is bounded by its zone at each end, not by one offset — so the day the clocks change is 23 or 25 hours long | `scripts/aw_client.py` `utc_bounds()` docstring | `tests/scenarios.py` `daylight-saving-transition-day`, held byte-for-byte by `test_scenarios.py`; `test_aw_client.py` at the function | `references/activitywatch.md` §"Time zones", `SKILL.md` "Timezone" |
