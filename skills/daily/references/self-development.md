...
| The transition instant carries two clock readings — `03:00` as you reach it and `02:00` once it has passed — so an entry split there reads as an overlap and is not one. Closing the apparent overlap is what loses the repeated hour | `scripts/harvest_post.py` `refusal_for_a_straddled_change()`, which is the copy a run is shown at the moment it matters | `references/output-format.md` §Conventions (which carries the case the refusal cannot see, where a block starts or ends inside the hour rather than containing it), `SKILL.md` Step 9, `TESTING.md` §"A create straddling the fall-back was billed an hour short", `CHANGELOG.md`. This one has been wrong in prose once already — the first `output-format.md` version split at the break rather than at the transition and lost the exact hour it was written to save — so check the arithmetic sums to the elapsed time before changing any copy |
