...
| Every flag a script parses is also named in `SKILL.md`'s "Files in this skill" entry for it — presence only; what the flag *does* is written once, wherever it is best explained | the script's own parser | `SKILL.md` "Files in this skill", held against the parser by `tests/test_references.py` — the one row here a test keeps rather than a habit. The `reconcile` skill's `SKILL.md` is a *third* copy (it invokes `harvest_list.py --by-day` and names `--window` and `--full`) and nothing holds it; rename a flag and it rots silently |
