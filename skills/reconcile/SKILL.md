...
2. **Which days the machine was used**, as a date index: list the *folder names* under the screenshot directory, resolved first from `TIMESHEET_SCREENSHOTS_DIR`, or `~/Pictures/WorkScreenshots` where that is unset. Substitute the resolved path — reading the literal one on a machine that configured another finds an empty folder, and an empty index makes a healthy month look like a month nobody worked. **Resolve it in the Bash tool** (`echo "${TIMESHEET_SCREENSHOTS_DIR:-}"`) and paste the result into the PowerShell listing: the configuration is published to Bash tool calls alone, so `$env:TIMESHEET_SCREENSHOTS_DIR` reads empty in PowerShell whatever the user set — which produces exactly the empty index this bullet exists to prevent. One listing of directory names, not their contents:
   ```powershell
