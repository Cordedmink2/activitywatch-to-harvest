...
- **Create the case with the tested helper, not a raw API call:**
  ...
  ```
  It resolves the client name → account GUID, creates the Case via the Dataverse SDK, and reads back the auto-assigned ticket number. The **customer account determines the ticket prefix** (illustrative: CON→Connexis, CNM→Cone Marshall, WOR→World Vision), so you only supply client + title — don't try to set the number. Those examples govern *creating* a case. Going the other way — resolving an observed ticket number to its client — derive it by grouping `.mcp/harvest_assignments*.json` on `project.code` prefix → `client.name` rather than reading a list; `PSO` and `SLA` each span 9–14 clients, so prefix alone never decides those. It's a client-facing CRM, so confirm the resolved client + title with the user before adding `--yes`.
